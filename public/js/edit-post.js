...
document.querySelector('#submitPost').addEventListener('click', async () => {
	...
	const CKWrapper = document.querySelector(
		'.CKPost-wrapper-edit-post'
	).innerHTML;

	CKeditorPost = {
		postContent:
			gridTop.outerHTML + CKWrapper + embed.value + gridBottom.outerHTML,
	...
	const datas = await fetch(`/account/edit-post/${postID}`, {
		method: 'PUT',
		headers: {
